...
class GlobalParameters:
    def __init__(self, n_chunks = 1, chunksize = 5):
        self.count = 0
        self.n_chunks = n_chunks
        self.chunksize = chunksize
        self.total_non_nan = 0
        self.lingua_como_chave = True
...
EXAM_LC = ("TX_RESPOSTAS_LC", "TX_GABARITO_LC", "CO_PROVA_LC", 0)
...
adaptadas = [519, 523, 522, 526, 521, 525, 565, 520, 564] # isso tem que ser extraído a mão mesmo
...
class ChaveQuestao:
    def __init__(self, GP, cod_exame, pos, lingua = 0):
        self.cod = int(cod_exame)
        self.pos = int(pos)
        self.ling = lingua
        self.lingua_e_chave = GP.lingua_como_chave

    def chave(self):
        if self.lingua_e_chave:

            if 1 <= self.pos and self.pos <= 5:
                return (self.cod, self.pos, self.ling)
            else:
                return (self.cod, self.pos, np.nan)
        else:
            return (self.cod, self.pos)

codepos_to_idx = dict()
for row in itens_provas[['CO_ITEM', 'CO_PROVA', 'CO_POSICAO', 'TP_LINGUA']].iterrows():
    item, exam, pos = row[1]['CO_ITEM'], row[1]['CO_PROVA'], row[1]['CO_POSICAO']
    lingua = row[1]['TP_LINGUA']

    chave = ChaveQuestao(GP, exam, pos, lingua)

    codepos_to_idx[chave.chave()] = item
...
class RightAnswerRate:
    ...
    def process_chunk(self, chunk, GP):
        ...
        def check_answers(row):
            ...
            return pd.Series([answers, row[self.exam_code], row[self.tp_lingua]], index = ['answers', 'exam_code', 'lingua'])

        def conta_acertos(row):
            idx = 1
            answers, code, lingua = row['answers'], row['exam_code'], row['lingua']
            ...
            for i in answers:
                if i:
                    # TODO TEM MAIS ACERTOS QUE CANDIDATOS!!!
                    chave = ChaveQuestao(GP, code, i + self.offset, lingua)
                    item_idx = codepos_to_idx[chave.chave()]
        ...
        chunk_cortado = chunk.loc[:, [self.col_answers, self.col_gt, self.exam_code, self.tp_lingua]]
        check = chunk_cortado.apply(check_answers, axis = 1, result_type = 'expand')
    ...
    def output_result(self, GP):
        ...
        dict_data_out["Rosa"] = []
        dict_data_out["Lingua"] = []
        dict_data_out["taxa_acerto"] = []
        dict_data_out["n_acertos"] = []
        dict_data_out["n_candidatos"] = []

        # as provas Laranja e Verde são versões adaptadas das provas Branca/Cinza
        ...
        for (idx, count) in self.right_answers.items():
            ...
            for codepos in list_codepos:
                ...
                if GP.lingua_como_chave:
                    code, pos, ling = codepos
                else:
                    code, pos = codepos
                    ling = np.nan         
                ...
                # Se lingua é 1/0/NaN em alguma das tuplas, ela tem de ser igual em todas as outras
                lingua = ling
            ...
            dict_data_out["Lingua"].append(tp_lingua)
...
for chunk in pd.read_csv("DADOS/MICRODADOS_ENEM_2019.csv", sep=';', chunksize=GP.chunksize, usecols = columns):
    ...
    print("\r{0} blocos lidos (total de {2} provas lidas; {3} regular, {4} reaplicação)"
                .format(GP.count, GP.n_chunks, GP.total_non_nan, GP.total_regular, GP.total_reaplicacao), end = '')
